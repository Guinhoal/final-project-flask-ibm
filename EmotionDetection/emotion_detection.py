...
def sentiment_analyzer_total(text_to_analyse):  
    url = 'https://sn-watson-sentiment-bert.labs.skills.network/v1/watson.runtime.nlp.v1/NlpService/SentimentPredict'  
    myobj = {"raw_document": {"text": text_to_analyse}}  
    header = {"grpc-metadata-mm-model-id": "sentiment_aggregated-bert-workflow_lang_multi_stock"}
    response = requests.post(url, json=myobj, headers=header)
    ...
    if "documentSentiment" not in formatted_response:
        return json.dumps({
            "error": "documentSentiment not found in the response"
        })
    ...
    dominant_emotion = max(emotion_scores, key=emotion_scores.get)
    return json.dumps({
        "anger": round(emotion_scores['anger'], 3),
        "disgust": round(emotion_scores["disgust"], 3),
        "fear": round(emotion_scores["fear"], 3),
        "joy": round(emotion_scores["joy"], 3),
        "sadness": round(emotion_scores["sadness"], 3),
